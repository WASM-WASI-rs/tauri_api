...
fn Menu() -> impl IntoView {
    ...
    let menu_manual = create_local_resource(
        ...
        move |_| async move {
            ...
            let mut item_open =
                tauri_api::menu::item::MenuItem::with_id("Item 1 - Manual", "manual-item_1").await;
            let mut item_close =
                tauri_api::menu::item::MenuItem::with_id("Item 2 - Manual", "manual-item_2").await;
    ...
    let menu_with_items = create_local_resource(
        || (),
        move |_| async move {
            let mut item_open = tauri_api::menu::item::MenuItemOptions::new("Item 1 - w/ items");
            item_open.set_id("w_items-item_1");
            let mut item_close = tauri_api::menu::item::MenuItemOptions::new("Item 2 - w/ items");
            item_close.set_id("w_items-item_2");
            let items = vec![item_open.into(), item_close.into()];

            let (menu, mut listeners) =
                tauri_api::menu::Menu::with_id_and_items("tauri-sys_menu_w_items", items).await;
